CREATE TABLE public.dependents (
  ...
  name text NOT NULL,
  nickname text,
  relation text NOT NULL DEFAULT 'filho',
  birth_date date,
  school text,
  monthly_allowance numeric(14,2),
  color text,
  notes text,
...
CREATE POLICY "dependents_insert_own" ON public.dependents
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "dependents_update_own" ON public.dependents
  FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "dependents_delete_own" ON public.dependents
...
-- Backfill das novas categorias para quem já usa o sistema
DO $$
DECLARE
  u record;
BEGIN

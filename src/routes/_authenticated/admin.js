...
import { KeyRound, Loader2, Search, ShieldCheck, UserCog, Users, Database, TrendingUp } from "lucide-react";
...
import { adminCancelSubscription, adminDeleteUser, adminSetUserPassword, adminUpdateUser, } from "@/lib/admin-users.functions";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Textarea } from "@/components/ui/textarea";
import { fixComplexAdjustments, fixNexxusTransaction } from "@/lib/admin-fixes.functions";
import { useAuth } from "@/hooks/use-auth";
import { supabase } from "@/integrations/supabase/client";
import { adminOverview, adminResetUserPin, adminSaveSupportNotes, adminSetUserRole, adminSetUserStatus, } from "@/lib/admin.functions";
...
function AdminContent({ isAdmin }) {
    ...
    const filtered = useMemo(() => {
        ...
        return (profiles.data ?? []).filter((profile) => {
            ...
            return (name.includes(term) || email.includes(term) || (digits.length > 0 && cpf.includes(digits)));
        });
    ...
    return (<AppShell>
      <div className="space-y-4">
        ...
        {isAdmin ? (<div className="grid gap-3 auto-cards-sm">
            ...
            <div className="flex flex-col gap-2 rounded-xl border border-dashed border-border p-3">
              ...
              <Button variant="outline" size="sm" onClick={async () => {
                try {
                    ...
                    if (res.count > 0)
                        toast.success("Lançamento Nexxus corrigido!");
                    else
                        toast.info("Lançamento não encontrado ou já corrigido.");
                }
                catch (e) {
        ...
        <Tabs defaultValue="users">
          ...
          <TabsContent value="users" className="mt-4 space-y-3">
            <div className="flex flex-wrap gap-3">
              <div className="relative min-w-64 flex-1">
                ...
                <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nome, CPF ou e-mail" className="pl-9"/>
              </div>
              ...
              <Table>
                ...
                <TableBody>
                  {profiles.isLoading ? (<TableRow>
                      ...
                      <TableCell colSpan={7} className="py-8 text-center text-sm text-muted-foreground">
                    ...
                    </TableRow>) : (filtered.map((profile) => (<TableRow key={profile.id}>
                        ...
                        <TableCell>
                          <div className="flex flex-wrap gap-1">
                            {(rolesByUser.data?.get(profile.user_id) ?? ["user"]).map((role) => (<Badge key={role} variant={role === "user" ? "secondary" : "default"} className="text-[10px]">
                                {role}
                              </Badge>))}
                          </div>
                        </TableCell>
          ...
          {isAdmin ? (<TabsContent value="ai" className="mt-4 space-y-4">
              <AiSettingsPanel />
              <TrialGrantPanel />
              <TrialLicensesPanel />
            </TabsContent>) : null}

          {isAdmin ? (<TabsContent value="security" className="mt-4 space-y-4">
              <AdminAccessPanel />
              <BlockedIpsPanel />
            </TabsContent>) : null}

          {isAdmin ? (<TabsContent value="categories" className="mt-4 space-y-4">
          ...
          <TabsContent value="logs" className="mt-4">
            <div className="overflow-x-auto rounded-xl border border-border bg-card">
              <Table>
                ...
                <TableBody>
                  {(logs.data ?? []).length === 0 ? (<TableRow>
                      <TableCell colSpan={5} className="py-8 text-center text-sm text-muted-foreground">
                        Nenhuma ação registrada ainda.
                      </TableCell>
                    </TableRow>) : ((logs.data ?? []).map((log) => (<TableRow key={log.id}>
      ...
      <ManageUserDialog profile={selected} canEdit={isAdmin} isSelf={selected?.user_id === user?.id} roles={selected ? (rolesByUser.data?.get(selected.user_id) ?? []) : []} onClose={() => setSelected(null)} onChanged={refreshAll}/>
    </AppShell>);
}
function ManageUserDialog({ profile, canEdit, isSelf, roles, onClose, onChanged, }) {
    ...
    return (<Dialog open={Boolean(profile)} onOpenChange={(value) => (value ? null : onClose())}>
      <DialogContent className="max-w-lg">
        ...
        {!profile ? null : !canEdit ? (<p className="text-sm text-muted-foreground">
            ...
            <section>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {["active", "suspended", "canceled"].map((status) => (<Button key={status} size="sm" variant={profile.status === status ? "default" : "outline"} disabled={pending !== null} onClick={() => run(`status-${status}`, () => adminSetUserStatus({
                    data: { targetUserId: profile.user_id, status },
                }), "Situação atualizada")}>
                    {pending === `status-${status}` ? (<Loader2 className="mr-2 size-4 animate-spin"/>) : null}
            ...
            <section>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                return (<Button key={role} size="sm" variant={has ? "default" : "outline"} disabled={pending !== null || (isSelf && role === "admin" && has)} onClick={() => run(`role-${role}`, () => adminSetUserRole({
                        data: { targetUserId: profile.user_id, role, grant: !has },
                    }), has ? "Papel removido" : "Papel concedido")}>
                      {pending === `role-${role}` ? (<Loader2 className="mr-2 size-4 animate-spin"/>) : null}
            ...
            <section>
              ...
              <form className="mt-2 flex gap-2" onSubmit={(event) => {
                ...
                void run("pin", () => adminResetUserPin({ data: { targetUserId: profile.user_id, pin } }), "Senha redefinida");
            }}>
                <Input id="admin-pin" name="pin" inputMode="numeric" maxLength={6} placeholder="000000" className="tracking-[0.3em]"/>
                <Button type="submit" disabled={pending !== null}>
                  {pending === "pin" ? (<Loader2 className="size-4 animate-spin"/>) : (<KeyRound className="size-4"/>)}
                </Button>
              </form>
            </section>
            ...
            <section>
              ...
              <form className="mt-2 grid gap-2 sm:grid-cols-2" onSubmit={(event) => {
                event.preventDefault();
                const form = new FormData(event.currentTarget);
                void run("profile", () => adminUpdateUser({
                    data: {
                ...
                <Input name="contact_email" type="email" defaultValue={profile.contact_email ?? ""} placeholder="E-mail de contato"/>
                <Input name="phone" defaultValue={profile.phone ?? ""} placeholder="Telefone"/>
                <Input name="login_email" type="email" placeholder="Novo e-mail de acesso (opcional)" className="sm:col-span-2"/>
            ...
            <section>
              ...
              <form className="mt-2 flex gap-2" onSubmit={(event) => {
                ...
                void run("password", () => adminSetUserPassword({
                ...
                <Button type="submit" variant="outline" disabled={pending !== null}>
                  {pending === "password" ? (<Loader2 className="size-4 animate-spin"/>) : (<KeyRound className="size-4"/>)}
                </Button>
              </form>
            </section>
            ...
            <section className="rounded-xl border border-destructive/40 bg-destructive/5 p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <Button size="sm" variant="outline" disabled={pending !== null} onClick={() => {
                if (!window.confirm("Cancelar a assinatura e voltar este usuário ao plano gratuito?"))
                    return;
                void run("cancel", () => adminCancelSubscription({ data: { targetUserId: profile.user_id } }), "Assinatura cancelada");
            }}>
                  {pending === "cancel" ? <Loader2 className="mr-2 size-4 animate-spin"/> : null}
                  Cancelar assinatura
                </Button>
                <Button size="sm" variant="destructive" disabled={pending !== null || isSelf} onClick={() => {
                const typed = window.prompt('Excluir a conta e TODOS os dados deste usuário? Digite EXCLUIR para confirmar.');
                if (typed !== "EXCLUIR")
                    return;
                void run("delete", () => adminDeleteUser({
                    data: { targetUserId: profile.user_id, confirmation: "EXCLUIR" },
                }), "Conta excluída").then(onClose);
            }}>
            ...
            <section>
              ...
              <form className="mt-2 space-y-2" onSubmit={(event) => {
                ...
                void run("notes", () => adminSaveSupportNotes({
                    data: { targetUserId: profile.user_id, notes },
                }), "Anotação salva");
            }}>
                <Textarea id="admin-notes" name="notes" rows={3} defaultValue={profile.support_notes ?? ""} placeholder="Histórico de atendimento, acordos, pendências…"/>
                <DialogFooter>
                  <Button type="submit" variant="outline" disabled={pending !== null}>
